...
sm = fits.open(os.path.join(os.environ["MD10"], 'output_MD_1.0Gpc.fits'))[1].data	

for ii in range(len(sm)):
	print sm[ii]
	fileList = n.array(glob.glob(os.path.join(os.environ["MD10"], 'work_agn', 'out_'+sm['snap_name'][ii]+'_SAM_Nb_?.fits')))
	z = sm['redshift'][ii]
	print fileList
